...
*/
with TRUCK_USING_FEE AS(
    SELECT a.HISTORY_ID, b.CAR_TYPE, TIMESTAMPDIFF(day, a.START_DATE, a.END_DATE) + 1 AS USING_DAY, b.DAILY_FEE * (TIMESTAMPDIFF(day, a.START_DATE, a.END_DATE) + 1) AS USING_FEE
    FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY a
    JOIN CAR_RENTAL_COMPANY_CAR b ON a.CAR_ID = b.CAR_ID
    WHERE b.CAR_TYPE = "트럭" 
),TRUCK_DISCOUNT AS(
    SELECT CONVERT(SUBSTRING_INDEX(DURATION_TYPE, '일', 1), SIGNED) AS DURATION, DISCOUNT_RATE
    FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN
    WHERE CAR_TYPE = "트럭"
), TRUCK_DISCOUNT_FEE AS (
    SELECT HISTORY_ID, USING_FEE AS FEE
    FROM TRUCK_USING_FEE
    WHERE USING_DAY < 7

    UNION

    SELECT HISTORY_ID, ROUND(USING_FEE * ((100 - (SELECT DISCOUNT_RATE FROM TRUCK_DISCOUNT WHERE DURATION = 7)) / 100), 0) AS FEE
    FROM TRUCK_USING_FEE
    WHERE USING_DAY >= 7 and USING_DAY < 30

    UNION

    SELECT HISTORY_ID, ROUND(USING_FEE * ((100 - (SELECT DISCOUNT_RATE FROM TRUCK_DISCOUNT WHERE DURATION = 30)) / 100),0) AS FEE
    FROM TRUCK_USING_FEE
    WHERE USING_DAY >= 30 and USING_DAY < 90

    UNION

    SELECT HISTORY_ID, ROUND(USING_FEE * ((100 - (SELECT DISCOUNT_RATE FROM TRUCK_DISCOUNT WHERE DURATION = 90)) / 100),0) AS FEE
    FROM TRUCK_USING_FEE
    WHERE USING_DAY >= 90
)
SELECT HISTORY_ID, FEE
FROM TRUCK_DISCOUNT_FEE
ORDER BY FEE DESC, HISTORY_ID DESC
